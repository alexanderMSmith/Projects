...
public class Pizza {
    ...
    public Pizza(String s, int meat, int vegs, boolean v) {
        ...
        if(meat > 0){
            numberOfMeat = meat; //Sets meat if input is valid
        }
        
        if(vegs > 0){
            numberOfVeggies = vegs; //Sets veggies if input is valid
        }
        
        vegan = v;
        
        if(vegan){ //Deletes meats if vegan
            numberOfMeat = 0;
        }
    ...
    public void setMeat(int meat) {
        if(meat > 0 && vegan == false){ //If input is valid and pizza is not vegan.
            numberOfMeat = meat;
        }
    }
    
    public void setVeg(int vegs) {
        if(vegs > 0){ //if input is valid set value
    ...
    public void setVegan(boolean v) {
        vegan = v;
        if(v = true){ //If turned into vegan, remove meats.
            numberOfMeat = 0;
        }
    }
    ...
    public double calcCost() {
        double cost = 0.0;
        if(size.equalsIgnoreCase("small")){ //Determine pizza type
            cost += 10;
        ...
        double meatCost = numberOfMeat * 2;
        double veggieCost = numberOfVeggies;
        
        cost = cost + meatCost + veggieCost; //Add cost of all toppings
         
        if(vegan){ //Check if the pizza's vegan
            cost+=2;
        }
    ...
    public String getDescription() {
        String isVegan = "not vegan. ";
        if(vegan == true){ //Check if vegan friendly
            isVegan = "vegan friendly. ";
        }
        return size + " pizza, " + isVegan + numberOfMeat + " meat toppings " + numberOfVeggies + " veggie topping. $" + calcCost() + " dollars.";
    }
}
